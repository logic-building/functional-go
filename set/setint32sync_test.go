...
func TestSetInt32SyncAdd(t *testing.T) {
	...
	mySet := NewSetInt32Sync(list)
	mySet.Add(40)
	mySet.Add(40)

	for _, num := range mySet.GetList() {
		if !list_op.SomeInt32(num, expected) {
...
func TestSetInt32SyncJoin(t *testing.T) {
	list := []int32{10, 20, 30, 20}
	mySet1 := NewSetInt32Sync(list)

	list = []int32{30, 40, 50}
	mySet2 := NewSetInt32Sync(list)
...
func TestSetInt32SyncSuperset(t *testing.T) {
	mySet1 := NewSetInt32Sync([]int32{10, 20, 30, 20})
	mySet2 := NewSetInt32Sync([]int32{10, 20})
	...
	mySet1 = NewSetInt32Sync([]int32{10, 20, 30, 20})
	mySet2 = NewSetInt32Sync([]int32{10, 20, 30, 20})

	if !mySet2.Superset(mySet1) {
		t.Errorf("TestSetInt32SyncSuperset failed. Expected=true, Actual=false")
	}

	mySet1 = NewSetInt32Sync([]int32{10, 20, 30, 20, 40})
	mySet2 = NewSetInt32Sync([]int32{10, 20, 30, 20})
...
func TestSetInt32SyncMultipleGoRoutine(t *testing.T) {
	mySet := NewSetInt32Sync([]int32{1000})

	var wg sync.WaitGroup
	wg.Add(2)

	go func(mySet *SetInt32Sync, wg *sync.WaitGroup) {
		defer wg.Done()
