...
func PMapIO() string {
	return `
// PMap<FINPUT_TYPE><FOUTPUT_TYPE> applies the function(1st argument) on each item in the list and returns a new list.
...
func PMap<FINPUT_TYPE><FOUTPUT_TYPE>(f func(<INPUT_TYPE>) <OUTPUT_TYPE>, list []<INPUT_TYPE>, optional ...Optional) []<OUTPUT_TYPE> {
	if f == nil {
		return []<OUTPUT_TYPE>{}
	}
	...
	if optional != nil && len(optional) > 0 {
		...
		if optional[0].RandomOrder == true {
			return pMap<FINPUT_TYPE><FOUTPUT_TYPE>NoOrder(f, list, worker)
		}
	}

	return pMap<FINPUT_TYPE><FOUTPUT_TYPE>PreserveOrder(f, list, worker)
}

func pMap<FINPUT_TYPE><FOUTPUT_TYPE>PreserveOrder(f func(<INPUT_TYPE>) <OUTPUT_TYPE>, list []<INPUT_TYPE>, worker int) []<OUTPUT_TYPE> {
...
func pMap<FINPUT_TYPE><FOUTPUT_TYPE>NoOrder(f func(<INPUT_TYPE>) <OUTPUT_TYPE>, list []<INPUT_TYPE>, worker int) []<OUTPUT_TYPE> {
...
func PMapIOErr() string {
	return `
// PMap<FINPUT_TYPE><FOUTPUT_TYPE>Err applies the function(1st argument) on each item in the list and returns a new list and error.
//  Order of new list is guaranteed. This feature can be disabled by passing: Optional{RandomOrder: true} to gain performance
//  Run in parallel. no_of_goroutines = no_of_items_in_list or 3rd argument can be passed to fix the number of goroutines.
//
// Takes 3 inputs. 3rd argument is optional
...
//  2. List
//  3. optional argument - fp.Optional{FixedPool: <some_number>}
func PMap<FINPUT_TYPE><FOUTPUT_TYPE>Err(f func(<INPUT_TYPE>) (<OUTPUT_TYPE>, error), list []<INPUT_TYPE>, optional ...Optional) ([]<OUTPUT_TYPE>, error) {
	if f == nil {
	...
	if optional != nil && len(optional) > 0 {
		...
		if optional[0].RandomOrder == true {
			return pMap<FINPUT_TYPE><FOUTPUT_TYPE>ErrNoOrder(f, list, worker)
		}
	}

	return pMap<FINPUT_TYPE><FOUTPUT_TYPE>ErrPreserveOrder(f, list, worker)
}

func pMap<FINPUT_TYPE><FOUTPUT_TYPE>ErrPreserveOrder(f func(<INPUT_TYPE>) (<OUTPUT_TYPE>, error), list []<INPUT_TYPE>, worker int) ([]<OUTPUT_TYPE>, error) {
	chJobs := make(chan map[int]<INPUT_TYPE>, len(list))
	go func() {
		for i, v := range list {
			chJobs <- map[int]<INPUT_TYPE>{i: v}
...
func pMap<FINPUT_TYPE><FOUTPUT_TYPE>ErrNoOrder(f func(<INPUT_TYPE>) (<OUTPUT_TYPE>, error), list []<INPUT_TYPE>, worker int) ([]<OUTPUT_TYPE>, error) {
	chJobs := make(chan <INPUT_TYPE>, len(list))
	go func() {
		for _, v := range list {
			chJobs <- v
